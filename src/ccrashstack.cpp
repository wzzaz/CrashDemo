...
CCrashStack::CCrashStack(PEXCEPTION_POINTERS pException)
{
    ...
    CreateDumpFile(QString( "Exception.dmp" ).toStdWString().data());
}
...
void CCrashStack::CreateDumpFile(LPCWSTR lpstrDumpFilePathName)
{
    // 创建Dump文件
    //
    ...
    dumpInfo.ExceptionPointers = m_pException;
